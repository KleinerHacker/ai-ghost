...
/**
 * Developer tests for [TextDesignProperty].
 *
 * The property wraps the design of the body text of a project and offers the style it carries - and every field of
 ...
 */
class TextDesignPropertyTest {

    /** Stands for the design carrying this part, the object a parent property writes into. */
    private class Holder(var design: TextDesign?)

    private lateinit var holder: Holder
    private lateinit var property: TextDesignProperty
    ...
    private fun newDesign(): TextDesign = TextDesign(
        style = StyleData(FontData("Text Serif", 11, bold = false, italic = false), Alignment.BLOCK)
    )

    /**
     * Use case: a view shows how the body text is drawn, so the style property and every field of it
     * answer with the values the wrapped object carries instead of a copy made earlier.
     */
    @Test
    fun readsTheStyleFromTheModelObject() {
        assertEquals("Text Serif", property.styleProperty.fontProperty.nameProperty.get())
        assertEquals(11, property.styleProperty.fontProperty.sizeProperty.get())
    ...
    fun exchangingTheModelObjectUpdatesEveryField() {
        ...
        property.set(TextDesign(style = StyleData(FontData("Other Serif", 44), Alignment.RIGHT)))

        assertEquals("Other Serif", property.styleProperty.fontProperty.nameProperty.get())
        assertEquals(Alignment.RIGHT, property.styleProperty.alignmentProperty.get())
