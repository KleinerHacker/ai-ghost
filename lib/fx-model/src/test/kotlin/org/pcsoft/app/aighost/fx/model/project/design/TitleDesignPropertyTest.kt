...
/**
 * Developer tests for [TitleDesignProperty].
 *
 * The property wraps the design of the title page of a project and offers the style it carries - and every field of
 ...
 */
class TitleDesignPropertyTest {

    /** Stands for the design carrying this part, the object a parent property writes into. */
    private class Holder(var design: TitleDesign?)

    private lateinit var holder: Holder
    private lateinit var property: TitleDesignProperty
    ...
    private fun newDesign(): TitleDesign = TitleDesign(
        style = StyleData(FontData("Title Serif", 28, bold = false, italic = false), Alignment.CENTER)
    )

    /**
     * Use case: a view shows how the title page is drawn, so the style property and every field of it
     * answer with the values the wrapped object carries instead of a copy made earlier.
     */
    @Test
    fun readsTheStyleFromTheModelObject() {
        assertEquals("Title Serif", property.styleProperty.fontProperty.nameProperty.get())
        assertEquals(28, property.styleProperty.fontProperty.sizeProperty.get())
    ...
    fun exchangingTheModelObjectUpdatesEveryField() {
        ...
        property.set(TitleDesign(style = StyleData(FontData("Other Serif", 44), Alignment.BLOCK)))
