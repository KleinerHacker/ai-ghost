...
/**
 * Property wrapping the design settings of the copyright page and offering every style it carries -
 * and every field of those styles - as a property of its own.
 *
 * The copyright page carries the copyright notice, the further copyright lines below it and, when
 * asked for, the author name; whether the page is printed at all is decided by the book, not here.
 *
 ...
 */
class CopyrightPageDesignProperty internal constructor() : SimpleObjectProperty<CopyrightPageDesign?>() {

    private val fields = BeanFields<CopyrightPageDesign> { fireValueChangedEvent() }

    /** Appearance of the copyright notice, as a property of its own. */
    val copyrightStyleProperty: StyleDataProperty = StyleDataProperty()

    /** Appearance of the copyright notice. */
    var copyrightStyle: StyleData?
        get() = copyrightStyleProperty.get()
        set(value) {
            copyrightStyleProperty.set(value)
        }

    /** Appearance of the further copyright lines, as a property of its own. */
    val copyrightAppendixStyleProperty: StyleDataProperty = StyleDataProperty()

    /** Appearance of the further copyright lines. */
    var copyrightAppendixStyle: StyleData?
        get() = copyrightAppendixStyleProperty.get()
        set(value) {
            copyrightAppendixStyleProperty.set(value)
        }

    /** Whether the author name is printed on the copyright page, as a property of its own. */
    ...
    init {
        fields.model(copyrightStyleProperty, "copyrightStyle", copyrightStyleProperty::refresh)
        fields.model(
            copyrightAppendixStyleProperty,
            "copyrightAppendixStyle",
            copyrightAppendixStyleProperty::refresh
        )
