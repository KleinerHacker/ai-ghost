...
/**
 * Property wrapping the design settings of the chapter pages and offering every style they carry - and
 * every field of those styles - as a property of its own.
 *
 * A chapter is a written part like the prolog or the epilog: it carries a heading, the further heading
 * lines below it and the body text, each set with its own style. Beyond that a chapter may push its
 * heading onto a page of its own, which is a switch of its own here.
 *
 ...
 */
class ChapterPageDesignProperty internal constructor() : SimpleObjectProperty<ChapterPageDesign?>() {

    private val fields = BeanFields<ChapterPageDesign> { fireValueChangedEvent() }

    /** Appearance of the chapter heading, as a property of its own. */
    ...
    /** Appearance of the further heading lines of the chapter, as a property of its own. */
    val titleAppendixStyleProperty: StyleDataProperty = StyleDataProperty()

    /** Appearance of the further heading lines of the chapter. */
    ...
    /** Appearance of the chapter body text, as a property of its own. */
    val textStyleProperty: StyleDataProperty = StyleDataProperty()

    /** Appearance of the chapter body text. */
    ...
    /** Whether the chapter heading stands on a page of its own, as a property of its own. */
    val titleOnSeparatePageProperty: BooleanProperty = SimpleBooleanProperty()

    /** Whether the chapter heading stands on a page of its own. */
    var titleOnSeparatePage: Boolean
        get() = titleOnSeparatePageProperty.get()
        set(value) {
            titleOnSeparatePageProperty.set(value)
        }
    ...
    init {
        ...
        fields.boolean(titleOnSeparatePageProperty, "titleOnSeparatePage")
