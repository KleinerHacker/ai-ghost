 ...
 * here, so [document] falls back to an empty [Document] the same way a freshly created book does.
 */
 ...
 * @property document The manuscript's simPlay [Document], an empty [Document] by default. Jackson
 *   reads and writes it as a nested JSON object through [DocumentJacksonModule] - simPlay's raw model
 *   is `kotlinx.serialization`-only, so [DocumentSerializer] and [DocumentDeserializer] bridge it
 *   instead of Jackson seeing it as one of its own types.
 */
