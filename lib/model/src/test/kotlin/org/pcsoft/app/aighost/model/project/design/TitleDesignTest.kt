...
/**
 * Developer tests for [TitleDesign].
 */
class TitleDesignTest {
    ...
    fun defaultsToThePlainStyle() {
        assertEquals(StyleData(), TitleDesign().style)
    }

    /**
     * Use case: the user styled the title page and stores the project, so the font and the alignment
     * come back exactly as they were chosen.
     */
    @Test
    fun roundTripsTheStyle() {
        val design = TestData.titleDesign()

        val restored: TitleDesign = mapper.readValue(mapper.writeValueAsString(design))

        assertEquals(design, restored)
        assertEquals(28, restored.style.font.size)
    ...
    fun readsEmptyDocument() {
        val design: TitleDesign = mapper.readValue("{}")

        assertEquals(TitleDesign(), design)
    }
    ...
    fun ignoresUnknownProperties() {
        val design: TitleDesign = mapper.readValue("""{"outline":true}""")
