...
Registrierung) auf Basis von **pluggiat**, die generische Konfigurations-Oberfläche samt Persistenz,
je mitgeliefertem Provider ein eigenes Modul unter `lib/plugin/provider`, einen voll funktionierenden
Stub-Provider als Nachweis und eine MkDocs-Anleitung zur Entwicklung eigener AI-Provider-Plugins.
...
* `lib/plugin/system` (`ai-ghost-plugin-system`) – aus IP-01 hervorgegangen, aber ein Eigenbau
  (eigener `URLClassLoader` je Plugin, eigenes Manifest-Schema, eigene Vertragsprüfung). Wird durch
  **IP-10 (Pluggiat-Migration)** auf **pluggiat** (externes JVM-Plugin-Management-System, siehe
  `external-docs.md`) umgestellt, bevor IP-02 fortsetzt.
...
* CI – `.github/workflows/ci.yml` und `release.yml`; der `ci-pipeline`-Skill nennt einen Schritt
  „Verify plugin“, für den es heute keine Gradle-Aufgabe gibt. Die Umstellung auf pluggiat (IP-10)
  bringt zusätzlich `GITHUB_ACTOR`/`GITHUB_TOKEN` für den GitHub-Packages-Bezug in die Pipeline.
* Versionen – alle Artefakte des Repos tragen dieselbe Version aus dem Release; es gibt kein Modul
  mit eigener Versionierung.

Fehlt: die Umstellung von `lib/plugin/system` auf pluggiat (IP-10), ein annotiertes
Konfigurationsmodell samt seiner Auswertung, die zwei Plugin-Orte (eingebautes `plugins`-Verzeichnis,
einstellbares Nutzer-Verzeichnis) über pluggiat konfiguriert, eine Provider-Registrierung als
Projektion über pluggiat, ein Startschritt, die Persistenz einer Provider-Wahl samt Geheimnissen und
des Verzeichnispfads, eine Auswahl- und Konfigurations-Oberfläche, der Container `lib/plugin/provider`
samt Stub-Modul, die MkDocs-Anleitung und die vier konkreten Anbindungen.
...
* **Plugin-API.** Jede für Plugin-Implementierungen bereitgestellte API liegt im **bereits
  ...
  Abschluss, Fehler, Abbruch), eine Vertragsversion (geprüft über `AiProviderContract.VERSION`) und
  die Nennung des Konfigurationsmodells. Weiterhin keine Abhängigkeit über `kotlin.stdlib` hinaus.
  Registrierung, Loader, Selektion kommen **nicht** hinein.
...
* **`ConfigField`-Liste als interner Vertrag.** Der annotierte Weg ist nur eine Art, eine
  ...
  die Liste selbst. Die Konfigurations-UI verarbeitet **nur** diese Liste. Dieser Teil ist
  pluggiat-unabhängig und bleibt app-eigen.
...
* **Zwei Plugin-Orte.** (1) Das **eingebaute** Verzeichnis `plugins` im Installationsverzeichnis –
  ...
  **Nutzer**-Verzeichnis für zusätzliche Plugins, Vorgabe `~/.ai-ghost/plugins`, über die
  Preferences-UI änderbar. Beide Orte werden pluggiat als Plugin-Orte übergeben und beim Start
  gelesen; das eingebaute zuerst. Kollidieren zwei Provider auf derselben Id, gewinnt der zuerst
  geladene – also der eingebaute vor einem Nutzer-Plugin, und bei zwei Nutzer-Plugins das zuerst
  geladene. Eine Änderung des Nutzer-Verzeichnisses wirkt beim nächsten Start.
* **Plugin-Manager.** Das Modul `lib/plugin/system` (`ai-ghost-plugin-system`, zweites Modul unter
  `lib/plugin` neben `api`) bekommt die zu durchsuchenden Verzeichnisse **übergeben** (es liest weder
  Installationsort noch Preferences selbst) und reicht sie an **pluggiats `PluginManager`** weiter,
  der Discovery, isoliertes Laden (parent-last Classloader je Plugin) und Fehlerisolation übernimmt.
  Der Manager instanziiert daraus die Provider, leitet ihre `List<ConfigField>` per `kotlin-reflect`
  ab (`ConfigModelReader`, app-eigen) und füllt eine Provider-Registrierung als Projektion über die
  von pluggiat aufgelösten Extensions. Er prüft die Vertragsversion und lehnt ein zu altes oder zu
  neues Plugin ab. Abhängigkeitskette: `app/ui` → `ai-ghost-plugin-system` → pluggiat +
  `ai-ghost-plugin-api`. Ein defektes oder inkompatibles Plugin wird gemeldet und übersprungen, nie
  bricht es den Start ab. Die Migration von einem Eigenbau-Loader auf pluggiat ist Gegenstand von
  **IP-10**, das vor IP-02 fortgesetzt wird.
...
* **Mitgelieferte Provider – ein Modul je Provider.** Ein neuer Container `lib/plugin/provider`,
  ...
  isoliert durch pluggiats parent-last Classloader je Plugin – von den anderen getrennt:
...
* **Paketierung.** Eine Gradle-Aufgabe paketiert ein Plugin-JAR im von pluggiat erwarteten Layout und
  prüft es, die CI führt sie aus, die Architekturregel benennt `lib/plugin` mit `api`, `manager` und
  `provider/*`. Alle Artefakte tragen die Repo-Version; die Kompatibilität regelt die Vertragsversion.
* **MkDocs-Anleitung.** Unter dem Menüpunkt „Plugins“ eine Übersichtsseite zum Plugin-Mechanismus
  (inklusive der Rolle von pluggiat) und darunter ein Bereich „AI Provider“ mit drei Seiten: die API,
  die allgemeine Entwicklung und ein durchgearbeitetes Beispiel (der Stub).
...
* Jedes Plugin wird über pluggiats parent-last Classloader je Plugin geladen; eine Config-Klasse in
  einem nicht-modularen Plugin-JAR ist damit von `kotlin-reflect` von sich aus erreichbar. Ein
  modulares Plugin muss sein Config-Paket `opens` – die Autoren-Anleitung sagt das.
* Kein JavaFX in `lib/plugin/api`, `lib/plugin/system`, den Modulen unter `lib/plugin/provider`
...
* Das jlink-Image bleibt lauffähig; pluggiat lädt die Plugin-JARs neben der Modulschicht der
  Anwendung.
...
* Jede neue Drittanbieter-Abhängigkeit – HTTP-Client, LLM-SDK, native llama.cpp-Bindung, eine
  Bibliothek für den OS-Schlüsselbund – wird zuerst mit dem Nutzer abgestimmt. `pluggiat` selbst ist
  bereits als neue Drittanbieter-Abhängigkeit abgestimmt (IP-10, über GitHub Packages bezogen). Der
  übrige Kern des Features (IP-02 bis IP-05) kommt ohne weitere solche aus, mit Ausnahme einer
  möglichen Schlüsselbund-Bibliothek in IP-03.
* Alle Artefakte tragen dieselbe Version wie das übrige Repo; `ai-ghost-plugin-api` wird nicht
  ...
  Anwendung regelt eine **Vertragsversion** je Provider-Eintrag im Manifest, die nur bei einer
...
ai-ghost-plugin-api            (bestehendes Modul, nur kotlin.stdlib)
  ...
  Vertragsversion, Prompt-Strom-Vertrag,
  ...
  -- NUR plugin-seitige Typen; keine Registrierung, kein Loader, keine Reflection --
        ▲                                   ▲
        │ (nur zum Bauen)                   │
lib/plugin/provider/<name>           ai-ghost-plugin-system   (Modul lib/plugin/system)
  je Provider ein Modul + JAR:         bekommt Verzeichnisse übergeben, reicht sie an
   provider-stub (voll),               pluggiats PluginManager weiter (Discovery,
   provider-lm-studio, -openai,        parent-last Classloader je Plugin, Fehlerisolation),
   -anthropic, -llama-cpp             prüft Vertragsversion, führt Provider-Registrierung
  -> jeweils ins eingebaute plugins/   als Projektion, ConfigModelReader (kotlin-reflect)
                                                       ▲
                                                       │
                                pluggiat (externe Abhängigkeit, GitHub Packages)
                                  Discovery, isoliertes Laden, Sicherheitsstrategie, Lifecycle
                                                       ▲
                                                       │
                                app/ui ────────────────┘
...
Kette: app/ui -> ai-ghost-plugin-system -> pluggiat + ai-ghost-plugin-api
...
* **`ai-ghost-plugin-api`** – der einzige Ort für plugin-seitige APIs. Bekommt den `AiProvider`-
  ...
  die Vertragsversion, `AiProviderConfig`-Marker, `@AiProviderConfigField`,
  `ConfigField` + `ConfigFieldType` neben `ProjectPart`.
* **`ai-ghost-plugin-system`** – Modul `lib/plugin/system`. Bekommt die zu scannenden
  Verzeichnisse übergeben (es kennt weder Installationsort noch `Preferences`) und konfiguriert damit
  **pluggiats `PluginManager`**, der den Verzeichnis-Scan, das isolierte Laden (parent-last
  Classloader je Plugin) und die Fehlerisolation je Plugin übernimmt. Der eigene Modulcode hält die
  Prüfung der Vertragsversion, die Provider-Registrierung mit der Regel „zuerst geladen gewinnt“ bei
  Id-Kollision (als Projektion über pluggiats aufgelöste Extensions), `ConfigModelReader`
  (Annotations → `List<ConfigField>` per `kotlin-reflect`, app-eigen) und das Befüllen/Auslesen einer
  `AiProviderConfig`-Instanz. Hängt von `ai-ghost-plugin-api`, `pluggiat` und `kotlin-reflect` ab,
  trägt kein JavaFX.
* **`lib/plugin/provider/<name>`** – je mitgeliefertem Provider ein eigenes Gradle-Modul
  ...
  Abhängigkeit mit, die durch pluggiats parent-last Classloader je Plugin von den anderen getrennt
  bleibt. `en`/`de`-`ResourceBundle`s für die Feldbeschriftungen.
...
tragen die konkreten Anbindungen, hängen am Ende und setzen den Kern (IP-10, IP-02 bis IP-05) nicht
als Nutzen voraus – der ist über den Stub bereits erbracht.

| ID    | Implementierungsplan                              | Anwendungsfall, der danach vollständig läuft                                                     | Abhängigkeiten |
|-------|--------------------------------------------------|-------------------------------------------------------------------------------------------------|----------------|
| IP-01 | Plugin API And Plugin Manager (**COMPLETED**, ersetzt durch IP-10) | Die Anwendung entdeckt und lädt Provider-Plugins beim Start; ein defektes Plugin wird übersprungen (nachgewiesen über ein Fixture-Plugin) | -              |
| IP-10 | Pluggiat-Migration (**COMPLETED**)                | Der Eigenbau-Loader aus IP-01 ist durch pluggiat ersetzt; dieselben Anwendungsfälle laufen über pluggiats `PluginManager` | -              |
| IP-02 | Provider Modules And Stub Provider                | Beim Start steht der ausgelieferte Stub-Provider in der Registrierung; `generate()` streamt seinen konfigurierten Text | IP-10          |
...
### IP-01: Plugin API And Plugin Manager (COMPLETED, ersetzt durch IP-10)

**Hinweis:** Der in diesem Plan gebaute Eigenbau-Loader (eigener `URLClassLoader`, eigenes
Manifest-Schema, `PluginClassRequirements`) wurde durch **IP-10 (Pluggiat-Migration)** ersetzt.
Dieser Abschnitt bleibt als historischer Nachweis stehen; maßgeblich für den aktuellen Aufbau von
`lib/plugin/system` ist IP-10.
...
* neues Modul `lib/plugin/system` (`ai-ghost-plugin-system`): nimmt eine **übergebene** Liste von
...
* `StartupStep` in `org.pcsoft.app.aighost.app.startup.step` (`app/ui`), rangiert hinter
  ...
  `app/ui` bekommt `requires org.pcsoft.app.aighost.plugin.system`.
...
`lib/plugin/api`, neues `lib/plugin/system`, `settings.gradle.kts`, `app/ui` (`build.gradle.kts`,
...
Ersetzt den Eigenbau-Loader aus IP-01 durch **pluggiat** (Discovery, isoliertes Laden,
Sicherheitsstrategie, Lifecycle); `AiProvider`-Vertrag, `AiProviderConfig`/`ConfigField` und
`ConfigModelReader` blieben app-eigen und unverändert. Der Implementierungsplan
(`FP-002-IP-10-PluggiatMigration.md`) ist mit Abschluss dieses Plans gemäß `development`-Regel
entfernt worden; sein Inhalt lebt hier und in `FP-002-AiProviderPlugins-status.md` weiter.

**Was abweichend vom ursprünglichen Plan umgesetzt wurde:**

* Das eigene Manifest-Schema (`PluginManifest`, `AiProviderDeclaration`,
  `plugin-manifest.schema.json`) entfiel vollständig statt nur teilweise – pluggiat parst und
  validiert `META-INF/plugin.yml` komplett selbst, inklusive eines intern verlangten, in der
  öffentlichen Doku nicht gelisteten Pflichtfelds `$version` (Migrationszweck) und eines Pflichtfelds
  `icon`.
  `AiProviderRegistry` bekam dafür eine neue `AiProviderExtensionConfig`
  (`@ExtensionPoint(key = "ai")`, implementiert `ExtensionConfiguration<AiProvider>`) mit den Feldern
  `implementation` (als `KClass<out AiProvider>`, nicht `String` – von pluggiat selbst aufgelöst),
  `id`, `name`, `contractVersion`.
* Scan-Strategie: `SingleJarScanStrategy` je Ort (nicht `MultiJarWithOwnFolderScanStrategy`), passend
  zu je einem Plugin-JAR pro Provider.
* `lib/plugin/system` braucht zusätzlich das Gradle-Plugin `org.javamodularity.moduleplugin`
  (bereits über `app/ui` im Repo etabliert), weil das pluggiat-JAR keinen `Automatic-Module-Name`
  trägt und ohne dieses Plugin nicht auf den JPMS-Modulpfad gelangt (`requires pluggiat;` in
  `module-info.java` schlägt sonst fehl).
* `build.gradle.kts` (Root) bekam ein zweites GitHub-Packages-Repository für pluggiat, analog zum
  bestehenden simPlay-Repository; die `licensee`-Konfiguration bekam zwei neue Ausnahmen: eine
  `allowUrl` für Bouncy Castles MIT-Lizenz-URL (transitive pluggiat-Abhängigkeit) und eine
  `allowDependency` für pluggiat selbst (das Artefakt trägt keine Lizenzmetadaten; andere
  KleinerHacker-Artefakte sind Apache-2.0, ebenso pluggiats Repository).
* **Testumfang bewusst verkleinert:** Ein ursprünglich gebauter Satz aus Fixture-Plugin-JARs, der
  pluggiats eigene Discovery/Lade-/Isolationsmechanik end-to-end nachtestete (`PluginManagerTest` samt
  `fixtures`-Sourceset), wurde wieder entfernt – das ist pluggiats eigene, bereits getestete
  Verantwortung, nicht die dieses Features. `lib/plugin/system` testet nur noch `AiProviderRegistry`
  (Id-Kollision „zuerst gewinnt“, `contractVersion`-Prüfung) und `ConfigModelReader` direkt, ohne über
  pluggiat zu laden.
* `PluginManager.load()` filtert ein nicht existierendes Verzeichnis selbst heraus, bevor es an
  pluggiat übergeben wird, da pluggiats Scan bei einem fehlenden Verzeichnis eine
  `NoSuchFileException` wirft statt leer zu bleiben.
* **Weitere Vereinfachung (nachträglich):** `lib/plugin/system` trägt jetzt ausschließlich
  `AiProviderExtensionConfig`. Der eigene `PluginManager`-Wrapper und `PluginLoadException` entfielen
  ersatzlos; `AiProviderRegistry` und `ConfigModelReader` zogen nach `app/ui`
  (`org.pcsoft.app.aighost.app.plugin`) um, ebenso ihre Tests. `app/ui`s `PluginLoadStartupStep` baut
  pluggiats `PluginManagerConfiguration`/`PluginManager` seither selbst auf und ruft
  `AiProviderRegistry.buildFrom(pluginManager)` direkt auf. `lib/plugin/system` verlor dadurch seine
  Abhängigkeiten auf `kotlin-reflect` und `slf4j-api`; `app/ui` bekam beide neu sowie eine direkte
  `pluggiat`-Abhängigkeit.

**Erwartetes Ergebnis (erreicht):** `./gradlew build` ist für das gesamte Repository grün, inklusive
`lib/plugin/system` mit den verbleibenden Tests.
...
* der neue Container `lib/plugin/provider` und das Modul `lib/plugin/provider/stub`
  ...
  `.claude/rules/architecture.md`; das Modul baut ein eigenes Plugin-JAR im von pluggiat erwarteten
  Layout, allein gegen `ai-ghost-plugin-api`. Ein Convention-/Gradle-Baustein, den die späteren
  Provider-Module (IP-06 bis IP-09) wiederverwenden.
* der **Stub-Provider**: Manifest-Eintrag mit Vertragsversion (Format aus IP-10), `StubProviderConfig`
  mit `responseText: String` (`@AiProviderConfigField`, Standardwert gesetzt) und optional
...
* Tests: der Stub wird vom Manager (über pluggiat) entdeckt, seine `List<ConfigField>` enthält
  `responseText`, `generate()` liefert den erwarteten Strom.
...
(IP-06 bis IP-09) getrennt, zusätzlich isoliert durch pluggiats parent-last Classloader je Plugin;
die Frage eines geteilten Klassenpfads stellt sich nicht. Der Stub bringt keine Abhängigkeit mit. Er
ersetzt den früher angedachten „Echo“-Provider und bleibt dauerhaft bestehen. Der wiederverwendbare
...
* `Preferences`: neue Gruppe `AiProviders` (gewählte Provider-Id + `providerId -> (feldName -> wert)`
  ...
  `fx-model` gespiegelt, mit Property-Tests; der Startschritt aus IP-01/IP-10 liest den Pfad jetzt von
  hier statt von der festen Vorgabe;
...
`ai-ghost-plugin-system` (direkter Testaufruf-Pfad, `ResourceBundle`-Lookup), `app/ui`
...
Enthalten: eine Gradle-Aufgabe, die ein Provider-Plugin-JAR im von pluggiat erwarteten Layout
paketiert und gegen die Vertragsversion prüft (der „Verify plugin“-Schritt des `ci-pipeline`-Skills),
angewandt auf jedes Modul unter `lib/plugin/provider`; die CI-Verdrahtung in `ci.yml`/`release.yml`
(inklusive `GITHUB_ACTOR`/`GITHUB_TOKEN` aus IP-10 für den pluggiat-Bezug); die geschärfte
...
* **Menüpunkt „Plugins“** mit einer **Übersichtsseite**: allgemeine Beschreibung des
  Plugin-Mechanismus – die Rolle von **pluggiat** (Discovery, isoliertes Laden, Sicherheitsstrategie),
  die beiden Plugin-Orte (das eingebaute `plugins`-Verzeichnis im Installationsverzeichnis, fix; das
  Nutzer-Verzeichnis, Vorgabe `~/.ai-ghost/plugins`, über die Preferences-UI änderbar, Wirkung beim
  nächsten Start; Vorrang des eingebauten bei gleicher Provider-Id, sonst „zuerst geladen gewinnt“),
  was ein Plugin-JAR mitbringen muss und wie ein defektes Plugin behandelt wird.
* Darunter ein Bereich **„AI Provider“** mit drei Seiten:
  1. **API** – Erklärung der AI-Provider-Plugin-API: `AiProvider` (Prompt-Strom-Vertrag, Handle,
     Abbruch, Fehler), Vertragsversion, `AiProviderConfig`,
  ...
  2. **Entwicklung** – allgemeine Beschreibung: welche Abhängigkeiten nötig sind (nur
     ...
     Config-Modell), wo und wie sie registriert werden (`configType`, Manifest-Eintrag zur
     Entdeckung über pluggiat, `opens` des Config-Pakets bei modularen Plugins), wie das JAR gebaut
     und wohin es gelegt wird, i18n über ein plugin-eigenes `ResourceBundle` (mindestens `en` und
     `de`, wie die mitgelieferten Provider).
...
und Auslieferung nicht auseinanderlaufen. Die Seiten beschreiben das Laden über pluggiat so, wie es
umgesetzt wurde (insbesondere die `opens`-Anforderung für modulare Plugins).
...
Provider-Modul-Baustein aus IP-02, mit Manifest-Eintrag, `LmStudioProviderConfig` (Basis-URL, Port,
...
Manifest-Eintrag, `OpenAiProviderConfig` (API-Schlüssel als `secret`, Modellname, Basis-URL für
...
Manifest-Eintrag, `AnthropicProviderConfig` (API-Schlüssel als `secret`, Modellname,
...
Manifest-Eintrag, `LlamaCppProviderConfig` (Pfad zur Modelldatei, Kontextgröße, Thread-Zahl,
...
```text
IP-01 (COMPLETED, ersetzt) ── IP-10 (COMPLETED) ── IP-02 ── IP-03 ─┬─ IP-04 ── IP-05  (Packaging/CI, dann Documentation)
                                                         ├─ IP-06  (LM Studio)
                                                         ├─ IP-07  (OpenAI)
                                                         ├─ IP-08  (Anthropic)
                                                         └─ IP-09  (llama.cpp)
...
* IP-10 ersetzt den Eigenbau-Loader aus IP-01 durch pluggiat, bevor IP-02 fortsetzt.
* Der Kern (IP-10, IP-02 bis IP-05) ist ohne echte LLM-Verbindung vollständig; der Nachweis läuft
  über den Stub aus IP-02, die Anleitung über den Stub aus IP-05.
...
* **Loader/Discovery/Isolation:** über **pluggiat** (externe Abhängigkeit, GitHub Packages, IP-10)
  statt Eigenbau – parent-last Classloader je Plugin, Sicherheitsstrategie
  (`InsecureSecurityStrategy` je Ort), Lifecycle. Eine Config-Klasse im nicht-modularen Plugin-JAR
  ist damit von sich aus reflektierbar; ein modulares Plugin muss `opens`.
* **Reflection:** `kotlin-reflect` im Manager-Modul, app-eigen, unverändert durch die
  pluggiat-Migration.
...
* **Provider-Paketierung:** je mitgeliefertem Provider ein eigenes Modul unter `lib/plugin/provider`
  ...
  Classloader-Isolation durch pluggiat – von den anderen getrennt; ein geteilter Klassenpfad
  entsteht nicht.
...
* **Versionierung:** alle Artefakte tragen die Repo-Version; `ai-ghost-plugin-api` wird nicht
  eigenständig versioniert. Die Kompatibilität regelt eine Vertragsversion je Provider-Eintrag im
  Manifest, die nur bei einem Bruch des Provider-Vertrags steigt und vom Manager geprüft wird.
...
* **Ermittlung des Installationsverzeichnisses** – der Ort von `<Install>/plugins` muss aus dem
  ...
  Entwicklungslauf ohne Image braucht einen Ersatz. In IP-01/IP-10 zu klären.
* **Genaues pluggiat-API und Manifest-Format** – siehe
  `FP-002-IP-10-PluggiatMigration.md`, Abschnitt „Weiterhin offen“.
...
Der Kern gilt als abgeschlossen, wenn IP-10 sowie IP-01 bis IP-05 erfüllt sind:
...
* Der **Stub-Provider** wird als Plugin-JAR des Moduls `lib/plugin/provider/stub` über das eingebaute
  Verzeichnis ausgeliefert, läuft durch denselben Plugin-Manager (auf Basis von pluggiat) wie ein
  fremdes Plugin, gibt seinen über die UI änderbaren Standardtext gestreamt zurück und bleibt
  dauerhaft bestehen. Er ist der vollständige Nachweis der Plugin-Funktionalität.
* Unter `lib/plugin/provider` besteht je mitgeliefertem Provider ein Modul mit eigenem Plugin-JAR;
  für Stub ist alles umgesetzt, für LM Studio, OpenAI, Anthropic und llama.cpp Config-Modell und UI,
...
* Eine Gradle-Aufgabe paketiert und prüft ein Provider-Plugin-JAR, die CI führt sie aus, und
  ...
  die Repo-Version.
* MkDocs trägt den Menüpunkt „Plugins“ mit einer Übersichtsseite zum Mechanismus (inklusive der
  Rolle von pluggiat) und dem Bereich „AI Provider“ mit den drei Seiten API, Entwicklung und
  Beispiel Stub; der „Build and verify MkDocs“-Schritt der CI bleibt grün.
* Jede plugin-seitige API liegt im bestehenden `ai-ghost-plugin-api` (nur `kotlin.stdlib`); es
  wurde kein weiteres plugin-seitiges Modul angelegt. `ai-ghost-plugin-system` hängt nur von
  `ai-ghost-plugin-api`, `pluggiat` und `kotlin-reflect` ab und lädt Plugins über pluggiats
  parent-last Classloader je Plugin. Kein `lib`-Modul außerhalb der Architektur-Ausnahmen bekommt
  JavaFX. `lib/ai` ist unverändert.
