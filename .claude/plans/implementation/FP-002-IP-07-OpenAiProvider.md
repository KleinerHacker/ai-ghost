...
* Unabhängig von IP-04, IP-05, IP-06, IP-08, IP-09.
...
* `lib/plugin/provider/openai` als `ai-ghost-provider-openai` in `settings.gradle.kts`.
* Provider-Modul-Baustein aus IP-02 verwenden.
* HTTP-Client bzw. OpenAI-SDK vorher mit dem Nutzer abstimmen; Lizenz auf der Allowlist.

### 2. Config-Modell

* `OpenAiProviderConfig`: `apiKey` (`secret`), `modelName`, `baseUrl`, `organization` (optional).
* `@AiProviderConfigField` je Feld; `en`- und `de`-`ResourceBundle`.

### 3. Client

* `generate()` gegen die Chat-Completions-API mit Streaming und Abbruch.
...
* Der OpenAI-Provider steht in der Auswahl; mit hinterlegtem Schlüssel liefert „Verbindung testen“ echten Text.
