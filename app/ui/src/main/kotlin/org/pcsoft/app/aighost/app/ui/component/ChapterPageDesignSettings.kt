...
/**
 * The "Chapter pages" design section: the appearance of the chapter heading, its further heading
 * lines and the body text, plus the switch that pushes the heading onto a page of its own.
 *
 ...
 */
class ChapterPageDesignSettings : BorderPane() {

    private val view: ChapterPageDesignSettingsView
    private val viewModel: ChapterPageDesignSettingsViewModel

    init {
        FluentViewLoader.fxmlView(ChapterPageDesignSettingsView::class.java).let {
            it.root(this)
            it.load().let { loaded ->
     ...
     */
    fun bindDesign(design: ChapterPageDesignProperty) {
